...
app.controller('homectrl', function ($http, $scope, $sce, $location, $window) {
    ...
    //const fastapi = "http://10.70.9.204:8000";
    ...
    $scope.onLoading = function () {
        ...
        $http.get(`${fastapi}/getalloteddata/` + $scope.branch + `/` + $scope.code).then(function (data) {
            ...
            if (data.data.length > 0) {
                localStorage.setItem(`${$scope.code}_blukdata2`, JSON.stringify(data.data))
    ...
    $scope.process_data = function (dt) {
        ...
        $scope.selected_class = 'New';
        $scope.loading = false;
        // console.log($scope.overall_emp_data)
    }
    ...
    if (!localStorage.getItem(`${$scope.code}_blukdata2`)) {
        $scope.onLoading();
    } else {
        $scope.emp_blukdata = JSON.parse(localStorage.getItem(`${$scope.code}_blukdata2`));
    ...
    $scope.updateStatus = function () {
        ...
        var obj =
        {
            ...
            "name_box": $scope.selected_student.name_box,
            "Mobile": $scope.selected_student.Mobile,
            "Name": $scope.selected_student.Name || "",
            "City": $scope.selected_student.City || "",
            "Address": $scope.selected_student.Address || "",
            "Reference": $scope.selected_student.Reference || "",
            "Paycode": parseInt($scope.selected_student.Paycode),
            "FacultyName": $scope.selected_student.FacultyName,
            "LatestStatus": $scope.selected_student.LatestStatus,
            "LatestUpdate": formattedDate,
            "Remarks": $scope.selected_student.Remarks || "",
            "interestedcourse":  $scope.selected_student.course
        }
        ...
        if (confirm("Are you sure you want to submit?")) {
            ...
            $http.post(`https://apis.aditya.ac.in/kafka/producer/whatsapp`, whatsappobj).then(function (data) {
                ...
                $http.post(`https://apis.aditya.ac.in/kafka/producer/leadstatusupdate`, leadobj).then(function (response) {
                    ...
                    // console.log($scope.emp_blukdata)                        
                    localStorage.setItem(`${$scope.code}_blukdata2`, JSON.stringify($scope.emp_blukdata))
                    $scope.process_data($scope.emp_blukdata)
