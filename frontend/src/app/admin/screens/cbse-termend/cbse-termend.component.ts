...
@Component({
  selector: 'app-cbse-termend',
  templateUrl: './cbse-termend.component.html',
  styleUrls: ['./cbse-termend.component.css'],
  providers: [DatePipe],

})
export class CbseTermendComponent implements OnInit {
  ...
  public exam_Marks_Sub_Entry: any = [];
  secInfo: any;
  show_form = 0;
  msg = "";
  loading = false;
  ...
  finalgrade: string;
  public showChildComponent: boolean = false;
  ...
  constructor(
    ...
    public datepipe: DatePipe, private http: HttpClient, private route: ActivatedRoute, private router: Router
  ) {
  ...
  getExams() {
    ...
    this.employee_Data = []
    this.layout_Type = []
    this.sec_Id = this.marks_entry_form.value.sectionid;
    var obj = {
    ...
    this._exam_marks_entry_Service.exams_list(obj).subscribe((data) => {
      ...
      if (data.length > 0) {
        ...
        // console.log(this.exam_List_Data)
  ...
  marks_entry_create() {
    ...
    // console.log(exam_id, section_id, this.exam_Sections_List)
      ...
      if (data.result.length > 0) {
        ...
        if (data.msg == "Marks Submitted" && data.examSubjects.length == data.subjectsData.length) {
          ...
          this._exam_marks_entry_Service.layout_type().subscribe((data: any) => {
            ...
            this.layout_Type = this.layout_Type.filter(
              (e: { exam_category: any }) =>
                e.exam_category == "TERM"
            );
            // console.log(this.layout_Type)
  ...
  SubmitMarks(obj: any) {
    ...
    this.router.navigate(['/termend_edit_or_update', this.exam_id, section_id, this.subject_name], {
      state: { updatedata: this.exam_Marks_Sub_Entry }
    });
    var preobj = {
      campus: obj.campus,
    ...
    this._exam_marks_entry_Service.exam_sub_marks(preobj).subscribe((data) => {
      ...
      if (data.status == true) {
        ...
        // console.log(this.exam_Marks_Sub_Entry)
  ...
  finalresult(examid: any, secid: any) {
    ...
    var filterlayouttype = this.layout_Type.filter(
      (e: { exam_category: any }) => e.exam_category == "TERM"
    );
    // console.log(filterlayouttype)
    var filterseclist = this.layout_Type.filter(
